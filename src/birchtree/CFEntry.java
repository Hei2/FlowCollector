...
public class CFEntry {
	...
	public CFEntry(double[] x, long index, int dstaddr) {
		this.n = 1;
		...
		indexList.add(index);
                ...
                dstaddrList = new HashSet<Integer>();
                dstaddrList.add(dstaddr);
	}
	...
	public CFEntry(CFEntry e) {
		...
		for(long i : e.getIndexList()) // this makes sure we get a deep copy of the indexList
			this.indexList.add(i);
                this.dstaddrList = new HashSet<Integer>();
		for(int i : e.getDstaddrList()) // this makes sure we get a deep copy of the dstaddrList
			this.dstaddrList.add(i);
	}
	
	protected ArrayList<Long> getIndexList() {
		return indexList;
	}
        
        protected HashSet<Integer> getDstaddrList() {
		return dstaddrList;
	}
	...
	protected int getChildSize() {
        ...
        protected int getBGPprefixnum() {                
                return dstaddrList.size();
	}
	...
	protected void update(CFEntry e) {
		...
		if(!this.hasChild()) { // we keep indexList only if we are at a leaf
			if(this.indexList!=null && e.indexList!=null)
                ...
                if(!this.hasChild()) { // we keep indexList only if we are at a leaf
			if(this.dstaddrList!=null && e.dstaddrList!=null)
	...
	public boolean equals(Object o) {
		...
		if(this.indexList!=null && !this.indexList.equals(e.indexList))
			return false;
                ...
                if(this.dstaddrList!=null && !this.dstaddrList.equals(e.dstaddrList))
			return false;
		return true;
	}
