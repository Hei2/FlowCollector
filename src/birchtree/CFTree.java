 ...
 *  Copyright (C) 2009 Roberto Perdisci (roberto.perdisci@gmail.com)
    Modified by Xia for experiment
 */
...
public class CFTree {
	/**
	 * Object for Leaf Entry counting
	 */
	public CFEntryCounter NumLeafEntries;
        
        public int LeafEntriesLimit;
	 ...
	 */
//	public boolean insertEntry(double[] x,long id) {
//		instanceIndex++;
//		
...
//	}
	 ...
	 * @return true if insertion was successful
	 */
	public boolean insertEntry(double[] x, long index, int dstaddr) {
            	instanceIndex++;		
		if(automaticRebuild && (instanceIndex % periodicMemLimitCheck)==0) {
			// rebuilds the tree if we reached or exceeded memory limits
			//rebuildIfAboveMemLimit();
	...
	private boolean insertEntry(CFEntry e) {
		...
		boolean dontSplit = root.insertEntry(e,NumLeafEntries);
		if(!dontSplit) {
			...
			if(automaticRebuild) {
                ...
                if(LeafEntriesLimit>0) {
                    rebuildIfLeafEntriesExceedLimit();
                }
...
//	}
	...
	public CFTree rebuildTree(int newMaxEntries, double newThreshold, int distFunction, boolean applyMergingRefinement, boolean discardOldTree) {
		...
		newTree.memLimit = this.memLimit;
                newTree.NumLeafEntries.copy(this.NumLeafEntries); 
	 ...
	 */
	public ArrayList<ArrayList<Long>> getSubclusterMembers() {
		ArrayList<ArrayList<Long>> membersList = new ArrayList<ArrayList<Long>>();
        ...
        public ArrayList<Long> getP2PtrafficID(int Thres) {
                    ...
                    if(!l.isDummy()) {
                        ...
                        for(CFEntry e : l.getEntries()) {
                            if(e.getBGPprefixnum() > Thres) { //number of BGP > thres
                                membersList.addAll(e.getIndexList());
                            }  
                        }
                    }
                    l = l.getNextLeaf();
	...
	public void printLeafIndexes() {
		...
		Long[] v = indexes.toArray(new Long[0]);
		Arrays.sort(v);
