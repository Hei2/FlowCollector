    ...
    private void SavePacket(DatagramPacket receivedPacket)
    {
                ...
                DataInputStream in = new DataInputStream(byteIn);)
        {
                ...
                Statement stmt = conn.createStatement();)
            {
                ...
                String insert = "INSERT INTO PACKET_V6_HEADER (count, sys_uptime, unix_secs, unix_nsecs, flow_sequence, engine_type, engine_id, sampling_interval) " +
                ...
                if (rs != null && rs.next())
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        insert = "INSERT INTO PACKET_V6 (header_id, srcaddr, dstaddr, nexthop, input, output, dPkts, dOctets, first, last, srcport, dstport, tcp_flags, prot, tos, src_as, dst_as, src_mask, dst_mask) " +
                ...
                {
                    System.out.println("Error: Unable to determine id of PACKET_V6_HEADER; associated flows dropped.");
                }
            }
            catch (SQLException ex)
            {
                System.out.println(ex.getMessage());
                System.out.println("Error: An SQLException occurred in PacketV6.\n");
            }
        }
        catch (IOException ex)
        {
            System.out.println(ex.getMessage());
            System.out.println("Error: An IOException occurred in PacketV6.\n");
        }
        catch (ClassNotFoundException ex)
        {
            System.out.println(ex.getMessage());
            System.out.println("Error: A ClassNotFoundException occurred in PacketV6.\n");
        }
    }
}